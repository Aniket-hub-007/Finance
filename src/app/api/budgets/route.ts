...
export async function GET() {
  try {
    ...
    return NextResponse.json({ success: true, data: budgets.map(b => ({ ...b, id: b._id.toString(), _id: b._id.toString() })) });
  } catch (error) {
    console.error(error);
...
export async function POST(request: Request) {
  try {
    const budget: Omit<Budget, 'id' | '_id'> = await request.json();
    const collection = await getCollection();
    const result = await collection.insertOne(budget);
    const newBudget = { ...budget, _id: result.insertedId, id: result.insertedId.toString() };
    return NextResponse.json({ success: true, data: newBudget }, { status: 201 });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ success: false, error: 'Failed to add budget' }, { status: 500 });
  }
}
